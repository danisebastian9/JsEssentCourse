"use strict"

//********************************
//*** Operadores Lógicos

var datoA = 10;
var datoB = 20;

// OPERADOR Y o AND - &&
var and = (datoA > 10 && datoB > 10) // Si ambas se cumplen true 
console.log('El resultado de la evaluación AND es: ' + and);

// OPERADOR O u OR - ||
var or = (datoA > 10 || datoB > 10); // Si cualquiera se cumple true
console.log('El resultado de la evaluación OR es: ' + or);

// OPERADOR DE NEGACIÓN o NOT - !
var not = !(datoA > 10) // Si esta condicion no se cumple
console.log('El resultado de la evaluación NOT es: ' + not);

